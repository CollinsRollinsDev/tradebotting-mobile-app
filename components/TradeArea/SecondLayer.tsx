...
const SecondLayer = () => {
    const { selectedCurrency, selectedTrade }:any = useSelector((state:any) => state.useTheReducer);
        ...
        <View style={styles.container}>
            ...
            <View style={styles.details}>
                <View style={styles.eachDetail}>
                    ...
                    <Text style={styles.item}>{currency}{coinData.current_price ? coinData.current_price.toLocaleString() : null}</Text>
                </View>

                <View style={styles.eachDetail}>
                    <Text style={styles.leftText}> Market Cap:</Text> 
                    <Text style={styles.item}>{currency}{coinData.market_cap ? coinData.market_cap.toLocaleString() : null}</Text>
                </View>

                <View style={styles.eachDetail}>
                    <Text style={styles.leftText}> 24hr Percent Change:</Text> 
                    <Text style={[styles.item, settingPriceColor]}>{coinData.price_change_percentage_24h ? coinData.price_change_percentage_24h.toFixed(2) : null}%</Text>
                </View>

                <View style={styles.eachDetail}>
                    <Text style={styles.leftText}> 24hr Price Change:</Text> 
                    <Text style={[styles.item, settingPercentColor]}>{currency}{coinData.price_change_24h ? coinData.price_change_24h.toLocaleString(undefined, {
